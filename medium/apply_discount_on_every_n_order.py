...
class Cashier:
  ...
  def __init__(self, n: int, discount: int, products: List[int], prices: List[int]):
    ...
    for index, product in enumerate(products):
      self.product_prices[product] = prices[index]
    

  def getBill(self, products: List[int], amount: List[int]) -> float:
    self.count += 1
    has_discount = self.count % self.n == 0
    cost = 0
    for index, product in enumerate(products):
      cost += self.product_prices[product] * amount[index]
    discount = (self.discount * cost) / 100 if has_discount else 0
