...
class TimeMap:
  ...
  def set(self, key: str, value: str, timestamp: int) -> None:
    if key in self._map:
      self._map[key].append(TimeStampedValue(timestamp, value))
    else:
      self._map[key] = [TimeStampedValue(timestamp, value)]
  ...
  def get(self, key: str, timestamp: int) -> str:
    if key not in self._map:
      return ''
    items = self._map[key]
    if timestamp >= items[-1].timestamp:
    ...
    while start <= end:
      ...
      elif items[middle].timestamp < timestamp:
        start = middle + 1
      else:
        end = middle - 1
    return items[middle - 1].value