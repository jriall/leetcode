...
class Solution:
  def lastStoneWeight(self, stones: List[int]) -> int:
    ...
    while len(sorted_stones) > 1:
      new_stone = abs(sorted_stones[-1] - sorted_stones[-2])
      sorted_stones.pop()
      sorted_stones.pop()
      insertion_index = len(sorted_stones)
      for index, num in enumerate(sorted_stones):
        if num >= new_stone:
          insertion_index = index
          break
      sorted_stones.insert(insertion_index, new_stone)
