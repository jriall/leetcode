...
class Solution:
  ...
  def __init__(self):
    ...
    for index, letter in enumerate(alphabet): 
      self.alphabet_hashtable[letter] = str(index)
  
  def isSumEqual(self, firstWord: str, secondWord: str, targetWord: str) -> bool:
    return self.get_word_value(firstWord) + self.get_word_value(secondWord) == self.get_word_value(targetWord)

  def getWordValue(self, word: str) -> int:
    return int(''.join([self.alphabet_hashtable[char] for char in word]))